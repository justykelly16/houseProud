...
if(!isset($_SESSION["id_40214842"]))
{
    ...
    $contractorid=mysqli_real_escape_string($conn,$_GET['conid']);
    
    $sql="SELECT * FROM  PRO_Job_alerts INNER JOIN PRO_Contractor ON PRO_Job_alerts.contractorfk=PRO_Contractor.id
        INNER JOIN PRO_User ON 
        PRO_Contractor.userfk=PRO_User.id WHERE PRO_Contractor.id='$contractorid';";
    $result = mysqli_query($conn, $sql) or die(mysqli_errno($conn));
    
    if(mysqli_num_rows($result)==1){
        $rows= mysqli_fetch_assoc($result);
        $name=$rows['name'];
        $services=$rows['Services_provided'];
        $bio=$rows['bio'];
        $transportation=$rows['Transportation'];
        $qual=$rows['Qualifications'];
        $county=$rows['County'];
         $town=$rows['Town'];
         $pic=$rows['profile_picture'];
         $countiescovered=$rows['Counties_Covered'];
         $contractoruserid=$rows['userfk'];
     ...
     $query1="SELECT AVG(Star_rating), COUNT(Star_rating) FROM PRO_Rate_Contractor 
      ...
      $query2="SELECT * FROM PRO_Rate_Contractor WHERE contractorfk='$contractorid'";
								...
								<div class="htlfndr-review-left-side">
									...
									<dl>
										...
										<dd><i class='fa fa-location-arrow'></i> <?php echo"$town, $county";?></dd>
                                                                                <?php if($id!=$contractoruserid){ ?>
                                                                                <dd><a href="#" data-toggle="modal" data-target="#htlfndr-sing-in"> <i class="fa fa-flag-o"></i> Report contractor</a></dd>
                                                                                <?php } ?>
										...
										<p class='htlfndr-Spagere-reviews'>(<span>$reviews</span> reviews)</p>
                                                                 ...
                                                                 ";
                                                                            ...
                                                                            if(strlen($bio)!= 0){
                                                                                ...
                                                                                echo"
                                                                               <p> No information available</p><br>";
                                                                            }
                                                                            ?>
											...
											<div class="row">
												...
												<div class="col-xs-6 col-sm-6 col-md-5 htlfndr-review-footer-marks">
													<h3>Transportation</h3>
                                                                                                        <?php if(strlen($transportation)!=0){
                                                                                                            echo"
													<p>$transportation</p>";
                                                                                                }else{
                                                                                                    echo"<p>Contractor has not added transportation yet</p>";
                                                                                                }?>
												</div><!-- .htlfndr-review-footer-marks -->
												<div class="col-xs-6 col-sm-6 col-md-5 htlfndr-review-footer-marks">
													<h3>Qualifications</h3>
													            <?php if(strlen($qual)!=0){
                                                                                                            echo"
													<p>$qual</p>";
                                                                                                }else{
                                                                                                    echo"<p>Contractor has not added any qualifications yet</p>";
                                                                                                }?>
												</div><!-- .htlfndr-review-footer-marks -->
                                                                                                <div class="col-xs-6 col-sm-6 col-md-5 htlfndr-review-footer-marks">
													<h3>Jobs Covered</h3>
														            <?php if(strlen($services)!=0){
                                                                                                            echo"
													<p>$services</p>";
                                                                                                }else{
                                                                                                    echo"<p>Contractor has not added any services yet</p>";
                                                                                                }?>
												</div><!-- .htlfndr-review-footer-marks -->
                                                                                                 <div class="col-xs-6 col-sm-6 col-md-5 htlfndr-review-footer-marks">
													<h3>Counties Covered</h3>
														            <?php if(strlen($countiescovered)!=0){
                                                                                                            echo"
													<p>$countiescovered</p>";
                                                                                                }else{
...
<div id="htlfndr-sing-in">
			<div class="htlfndr-content-card">
				<div class="htlfndr-card-title clearfix">
					<h2 class="pull-left">What would like to report</h2> 
				</div>
<form id="htlfndr-sing-in-form" name="htlfndr-sing-in-form" action="sendreport.php" method="POST">
					<h4>Please give us more information</h4>
                                       
                                        <br>
                                        <br>
                                        <div class="clearfix">
					
                                            <textarea id="comment" style="height:150px;" class="htlfndr-input " name="comment" placeholder="Comment(required)" required></textarea>
                                        <input type="hidden" name="site" value="viewconprofile.php?conid=<?php echo $contractorid ?>">
                                       <input type="hidden" name="con" value="<?php echo "$contractorid" ?>">
                                        <input type="hidden" name="reportfrom" value="<?php echo $id ?>">
                                        </div>
					<div class="clearfix">
						
						<input type="submit" name="report" id="report" value="Report" class="pull-right">
