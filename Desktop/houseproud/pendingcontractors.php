...
if(!isset($_SESSION["id_40214842"])&&(($_SESSION["admin_40214842"])=='No'))
{
    ...
    $sql="SELECT PRO_Contractor.id,PRO_Contractor.name, PRO_Contractor.Date_of_Birth, PRO_Contractor_verification.photo_id, 
        PRO_Contractor_verification.police_check,PRO_Contractor_verification.portfolio FROM 
        `PRO_Contractor_verification` INNER JOIN PRO_Contractor ON PRO_Contractor_verification.id=
        PRO_Contractor.id INNER JOIN PRO_User ON PRO_Contractor.userfk=PRO_User.id
        WHERE PRO_User.contractor='No'
";
	...
	<div class="container">
		...
		<main id="htlfndr-main-content" role="main">
				  ...
				  <?php if(mysqli_num_rows($result)>0){ ?>
						 ...
						 <tbody>
                                  <?php }else{
                                      echo"<h3>No Pending contractors</h3>";
                                  }?>
                                    <?php if(mysqli_num_rows($result)>0){
                                        while($row= mysqli_fetch_assoc($result)){
                                          ...
                                          $police=$row['police_check'];
                                          $photo=$row['photo_id'];
                                          $cv=$row['portfolio'];
                                          
                                          
                                          echo"<tr>
								<td class='htlfndr-scope-row'>$cname </td>
                                                                    <td data-title='DOB'>$dob</td>
								<td data-title='Police check'><a href='files/$police'>View</a></td>
								<td data-title='Photo ID'><a href='files/$photo'>View</a></td>
								<td data-title='CV'><a href='files/$cv'>View</a></td>
                                                                    <td><a href=acceptcontractor.php?cid=$cid><input type='submit' value='Accept' class='btn-primary'></a></td>
                                                                 <td><a href=rejectcontractor.php?cid=$cid><input type='submit' value='Reject' class='btn-primary'></a></td>
							</tr>
                                              ";
