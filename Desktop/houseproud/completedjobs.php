...
if(!isset($_SESSION["id_40214842"]))
{
    ...
    $query = "SELECT PRO_Post_Task.taskid, PRO_Post_Task.job_title, PRO_Post_Task.job_category,
        PRO_Post_Task.userfk, PRO_Jobs.status, PRO_Jobs.Date_posted, PRO_job_completed.completed_on, PRO_Jobs_assigned.price FROM PRO_Post_Task 
        INNER JOIN PRO_Jobs ON PRO_Post_Task.taskid = PRO_Jobs.id INNER JOIN PRO_Jobs_assigned ON PRO_Jobs.id=PRO_Jobs_assigned.id
        INNER JOIN PRO_job_completed ON PRO_Jobs_assigned.id=PRO_job_completed.id WHERE PRO_Post_Task.userfk = '$id'";
...
";
						 ...
						 <tbody>
							...
							<tr>
								...
								<td data-title='Completed'>"; echo timeAgo($time_ago); echo "</td>
								<td data-title='Cost'>&pound$cost</td>
								
                                                     <td>
                                                         <form method='POST' action='viewcompletedjob.php'>
						...
						<ul class='htlfndr-progress'>
							...
							<li>
								 <a href='#htlfndr-user-tab-1' class='htlfndr-step-description'>Customer</a>
                                    ...
                                    $sql ="SELECT PRO_Jobs.status, PRO_Jobs_assigned.price, PRO_job_completed.completed_on,
                                        PRO_Post_Task.job_title, PRO_Post_Task.taskid, PRO_Post_Task.job_category, 
                                        PRO_User.id, PRO_User.firstname, PRO_User.lastname FROM PRO_job_completed INNER JOIN PRO_Jobs_assigned
                                        ON PRO_job_completed.id=PRO_Jobs_assigned.id INNER JOIN PRO_Jobs ON PRO_Jobs_assigned.id=PRO_Jobs.id
                                        INNER JOIN PRO_Make_Offer ON PRO_Jobs.id = 
                                        PRO_Make_Offer.jobfk 
                                        INNER JOIN PRO_Post_Task ON PRO_Make_Offer.jobfk =PRO_Post_Task.taskid 
						 ...
						 <tbody>
                                            ";
                                                    while($row= mysqli_fetch_assoc($result3)){
                                                        ...
                                                        $jobcompleted=$row['completed_on'];
                                                        
                                                        $curtime=$jobcompleted;
							...
							<tr>
								...
								<td data-title='Category'>$clientjobcat</td>
                                                                    <td data-title='Completed'>"; echo timeAgo($time); echo "</td>
								<td data-title='Cost'>&pound$jobcost</td>
								
								<td>
                                                                        
                                                          
                                                            <form style='margin-top:0; display:inline;' method='POST' action='viewmycompletedjob.php'>
                                                             <input type='hidden' value='$clientjobid' name='jobid'>
                                                                  <input type='hidden' value='$contractorid' name='conid'>
