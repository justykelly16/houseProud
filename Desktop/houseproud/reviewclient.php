...
session_start();
include("connect.php");
if(($_SESSION["contractorid_40214842"])!=($_POST['contractor']))
{
...
$sql="SELECT * FROM PRO_rate_client WHERE jobfk='$job' AND userfk='$clientid'";
    $check = mysqli_query($conn, $sql);    
    
    if(mysqli_num_rows($check)==0){
             $query="SELECT job_title FROM PRO_Post_Task WHERE taskid='$job' ";
        $result= mysqli_query($conn, $query) or die(mysqli_errno($conn));
        $rows= mysqli_fetch_assoc($result);
        $jobtitle=$rows['job_title'];
        
        $query2="SELECT firstname, lastname FROM PRO_User WHERE id ='$clientid'";
         $result1= mysqli_query($conn, $query2) or die(mysqli_errno($conn));
      ...
      echo "<script>";
echo " alert('You have already left a review for this contractor');      
...
if(logout){
	...
	<div class="container">
		...
		<main id="htlfndr-main-content" role="main">
		  ...
		  <div class="row">
								<div class="col-md-6">	
                                                                    <form method="POST" class="htlfndr-review-form" action="postclientreview.php?userid=<?php echo"$id";?>">
                  
                                                                    	
                    <h3><?php echo"$jobtitle";?></h3><br>
                    <label>Client Name: <?php echo"$clientfirstname $clientlastname";?></label>
										...
										<input type="radio" name="htlfndr-review-radio-cleanliness" id="htlfndr-review-radio-5" value="5" checked="checked" />
                                                                                <input type="hidden" name="contractorid" value="<?php echo"$contractor";?>">
                                                                                <input type="hidden" name="jobid" value="<?php echo"$job";?>">
                                                                                 <input type="hidden" name="clientid" value="<?php echo"$clientid";?>">
                                                                                 <input type="hidden" name="name" value="<?php echo"$firstname $lastname";?>">
                                                                                  <input type="hidden" name="conid" value="<?php echo"$contractorname";?>">
                                                                        </div>
                                                               
                    
									
									<label for="review-text">review text</label>
                                                                        <textarea name="review-text" id="review-text" class="htlfndr-review-form-input" required="" ></textarea>
