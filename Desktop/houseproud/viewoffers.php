...
session_start();
include("connect.php");
if((($_SESSION["id_40214842"])!=($_POST['userid']))||(empty($_POST['jobid'])))
{
   ...
   $query="SELECT PRO_User.firstname, PRO_User.lastname, PRO_User.profile_picture, PRO_Make_Offer.contractorfk, PRO_Make_Offer.quote, PRO_Make_Offer.quote_accepted FROM PRO_Make_Offer
    INNER JOIN PRO_Contractor ON PRO_Make_Offer.contractorfk=PRO_Contractor.id INNER JOIN
    PRO_User ON PRO_Contractor.userfk=PRO_User.id WHERE PRO_Make_Offer.jobfk= '$jobid'";
...
if(logout){
	...
	<div class="htlfndr-user-avatar">
            ...
            <img src="images/<?php echo"$pic";?>" alt="user avatar" style="height:130; width:130"/>
						</div>
			...
			<div class="htlfndr-user-panel col-md-9 col-sm-8 htlfndr-credit-page">
                            ...
                            if(mysqli_num_rows($result)>0){
                                while($row= mysqli_fetch_assoc($result)){
                                    $contractorid=$row['contractorfk'];
                                    
                                    $contractorfname=$row['firstname'];
                                     $contractorlname=$row['lastname'];
                                                        $quote=$row['quote'];
                                                        $quotedecision=$row['quote_accepted'];
                                                        $conpic=$row['profile_picture'];
                                                        
                                                        
                                                       if($quotedecision=='Pending'){
						...
						<div class='htlfndr-person-card'>
							...
							<span class='htlfndr-person-name'><a href='viewconprofile.php?conid=$contractorid'>$contractorfname $contractorlname</a></span>
							<img class='pull-right htlfndr-user-avatar' style='margin-top:5px; margin-right:20px; width='50px'; height='50px'' src='images/$conpic' alt='user avatar' />
                                                            ";
                                                         $query1="SELECT AVG(Star_rating), COUNT(Star_rating) FROM PRO_Rate_Contractor 
                                                             WHERE contractorfk=$contractorid";
							...
							<h5 class='htlfndr-details'><span>Offer: </span> <span>&pound$quote</span><br>
						
                                                
                                                    
                                     <span class='pull-left'><a onclick='acceptFunction$contractorid()'><input type='submit' value='Accept' class='btn-success'>
                                        </a></span>
                                          <span class='pull-right'><a onclick='rejectFunction$contractorid()'><input type='submit' value='Reject' class='btn-danger'>
                                         </a></span>
                                      </h5>
                                                                   <script>
              
                                                        function acceptFunction$contractorid() {
                                                       var del = confirm('Are you sure want to accept offer from $contractorfname $contractorlname?');

                                                       if(del){
                                                            location.href = 'acceptoffer.php?jobid=$jobid&conid=$contractorid&userid=$id&quote=$quote';
                                                       }
                                                       }
                                                       </script>
                                                                                           <script>
              
                                                        function rejectFunction$contractorid() {
                                                       var del = confirm('Are you sure want to reject offer from $contractorfname $contractorlname?');

                                                       if(del){
                                                            location.href = 'rejectoffer.php?jobid=$jobid&conid=$contractorid&userid=$id';
                                                       }
                                                       }
                                                       </script>
                                ...
                                }else if(($quotedecision=='Rejected')&&($i==0)){
                                  echo"	<div class='htlfndr-credit-card'>
						<div class='htlfndr-add-card'>
							
								<div class='htlfndr-person-card'>
								<span class='htlfndr-person-name'>No More Offers Available</span>
							</div>
						</div>
					</div>	
                                        ";
					...
					<div class='htlfndr-credit-card'>
						<div class='htlfndr-add-card'>
								...
								<div class='htlfndr-person-card'>
								<span class='htlfndr-person-name'>No Offers made</span>
							</div>
						</div>
					</div>	
                                    ";
