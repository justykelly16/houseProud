   ...
   $admin=$_SESSION["admin_40214842"];
    ...
    $lastname=$_SESSION['lastname_40214842'];
     ...
     $query="SELECT PRO_User.id, PRO_User.firstname, PRO_User.lastname, PRO_Post_Task.taskid, 
         PRO_Post_Task.job_title, PRO_Post_Task.Description,
         PRO_Post_Task.street, PRO_Post_Task.town,PRO_Post_Task.county, 
         PRO_Post_Task.postcode, PRO_Post_Task.Due_date,PRO_Jobs.Date_posted, 
         PRO_Jobs.status, PRO_Contractor.name, PRO_Jobs_assigned.price,PRO_Jobs_assigned.contractorfk
         FROM PRO_User INNER JOIN PRO_Post_Task ON PRO_User.id= 
         PRO_Post_Task.userfk INNER JOIN PRO_Jobs ON PRO_Post_Task.taskid=
         PRO_Jobs.id INNER JOIN PRO_Jobs_assigned ON PRO_Jobs.id=PRO_Jobs_assigned.id
     ...
     $query1="SELECT PRO_User.id, PRO_User.firstname, PRO_User.lastname, PRO_Private_message.Message, PRO_Private_message.private_message_id, PRO_Private_message.timestamp
           FROM PRO_User INNER JOIN PRO_Private_message ON PRO_User.id = PRO_Private_message.userfk 
         WHERE PRO_Private_message.jobfk = '$jobid1' ORDER BY PRO_Private_message.timestamp DESC ";
    ...
    function send_message()
{
 ...
 return false;
      ...
      while($row= mysqli_fetch_assoc($result)){
				...
				<p class='htlfndr-details'><a href='userprofile.php?userid=$clientid'<span>Posted By: </span> <span>$clientfirstname $clientlastname</span></a></p>
                                    <p>"; echo timeAgo($time_ago); echo "</p>
                                        <p class='htlfndr-details'><a href='viewconprofile.php?conid=$contractorid'<span>Assigned Contractor: </span> <span>$contractorname</span></a></p>
				</div>
                                <div class='htlfndr-accordion-inner'>
                                <p class='htlfndr-details'><span><i class='fa fa-map-marker'></i> Location: </span> <span><br> $street <br>$town<br> $county<br>$postcode</span></p>
                        ...
                        echo"
                                               ...
                                               <form action='jobcomplete.php?userid=$id' method='POST' onsubmit='return jobComplete();'>
                                                   <input type='hidden' name='jobid' value='$jobid1'>
                                                       <input type='hidden' name='contractor' value='$contractorid'>
						<input type='submit'  class='htlfndr-book-now-button' value='Job Complete'>
					...
					<br>
                                                       ...
                                                       return confirm('Can you confirm that the job has been completed? Payment will processed on confirmation');
                                    ...
                                    while($row= mysqli_fetch_assoc($result1)){
                                        $messageid=$row['private_message_id'];
                                        $senderid=$row['id'];
                                        $fname=$row['firstname'];
                                        $lname=$row['lastname'];
                                        $message=$row['Message'];
                                        ...
                                        $query2="SELECT * FROM PRO_private_reply WHERE messagefk='$messageid'";
                                        ...
                                        <textarea name='reply_message$i' id='reply_message$i' rows='6' class='htlfndr-review-form-input' required></textarea>
                                            ...
                                            <input id='name$i' type='hidden' name='name$i' value='$firstname $lastname'>
                                                <input id='replyuser$i' type='hidden' name='replyuser$i' value='$id'>
                                                     ...
                                                     <input id='replyjobid$i' type='hidden' name='replyjobid$i' value='$jobid1'> 
 ...
 return false;
                     ...
                     $i++;    
                                    ...
                                    echo" <div class='htlfndr-accordion-inner'> 
                                        ...
                                        <textarea name='message' id='message' rows='6' class='htlfndr-review-form-input' placeholder='Ask a question' required></textarea>
